...
import com.ssafy.festival.FestivalMain;
import com.ssafy.shop.ShopMain;
...
public class TripInfoView {
	...
	private FestivalMain festivalMain;
...
//		});
	...
	public void setMain() {
		...
		JPanel rightTop2 = new JPanel(new GridLayout(1, 5));
		String[] item = { "검색조건선택", "관광지명", "주소" };
		findC = new JComboBox<String>(item);
		wordTf = new JTextField();
		searchBt = new JButton("검색");
		festivalBt = new JButton("지역 축제 정보");
		shopBt = new JButton("상권 정보");

		rightTop2.add(findC);
		rightTop2.add(wordTf);
		rightTop2.add(searchBt);
		rightTop2.add(festivalBt);
		rightTop2.add(shopBt);
...
//		searchBt.addActionListener( buttonHandler );
		// 참조코드 종료
		festivalBt.addActionListener(e -> openFestival());

		shopBt.addActionListener(e -> openShop());
	...
	public void openFestival() {
		festivalMain.setVisible(true);
		String s = "";
		if (curTrip.getLotAddress() != "") {
			s = curTrip.getLotAddress().substring(0, 2);
			if (s.equals("충청")) {
				s = curTrip.getLotAddress().substring(0, 1) + curTrip.getLotAddress().substring(2, 3);
			} else if (s.equals("전라")) {
				s = curTrip.getLotAddress().substring(0, 1) + curTrip.getLotAddress().substring(2, 3);
			} else if (s.equals("경상")) {
				s = curTrip.getLotAddress().substring(0, 1) + curTrip.getLotAddress().substring(2, 3);
			...
			s = curTrip.getStreetAddress().substring(0, 2);
			if (s.equals("충청")) {
				s = curTrip.getStreetAddress().substring(0, 1) + curTrip.getStreetAddress().substring(2, 3);
			} else if (s.equals("전라")) {
				s = curTrip.getStreetAddress().substring(0, 1) + curTrip.getStreetAddress().substring(2, 3);
			} else if (s.equals("경상")) {
				s = curTrip.getStreetAddress().substring(0, 1) + curTrip.getStreetAddress().substring(2, 3);
		...
		festivalMain.setTable(s);
	}

	public void openShop() {
		shopMain.setVisible(true);
		String s = "";
		if (curTrip.getLotAddress() != "") {
			s = curTrip.getLotAddress().substring(0, 2);
			if (s.equals("충청")) {
